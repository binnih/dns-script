...
A comprehensive DNS lookup tool for single domains or bulk lists. Covers standard record queries plus WHOIS, mail security auditing, SSL/TLS inspection, blacklist checking, CDN detection, port scanning, propagation checking, DNSSEC validation, and more.
...
# From a file — one domain per line, lines starting with # are ignored
...
| `--resolver IP/HOST` | DNS resolver to use (IP or hostname, default: system) |
...
| `--summary` | Print a one-line-per-domain summary table at end |
...
| Flag | Description |
|------|-------------|
| `--watch N` | Re-query every N seconds and highlight changes |
...
Use `-r` to query specific types only:

```bash
python3 dns_lookup.py example.com -r A MX TXT
```
...
**DKIM** — detects the mail provider from MX/SPF and probes its known selectors first, then 33 common ones. Flags revoked keys (empty `p=`) and selectors whose CNAME is published but resolves to nothing — in Microsoft 365 that normally means the `selector1`/`selector2` CNAMEs were created but DKIM signing was never enabled for the domain. Selectors cannot be enumerated via DNS, so no match is reported as inconclusive rather than as "no DKIM".  
...
### Zone transfer (`--axfr`)
Attempts AXFR against every NS. A successful transfer is flagged as a security risk and the returned records are dumped. Refused = pass.
...
Resolves each MX host and tests ports 25 (SMTP), 465 (SMTPS), and 587 (Submission) for connectivity.

### Port scan (`--portscan`)
Tests 20 common ports on each A record IP: 21/FTP, 22/SSH, 25/SMTP, 53/DNS, 80/HTTP, 443/HTTPS, 3306/MySQL, 3389/RDP, 5432/PostgreSQL, 6379/Redis, and more. Flags sensitive ports that are open.
...
Queries 6 public resolvers + authoritative NS in parallel and compares answers to consensus. Mismatches highlighted in red. Defaults to A/AAAA/MX/NS; use `-r` to specify types.
...
### Watch mode (`--watch N`)
Re-queries every N seconds and prints only what changed — added values in green, removed in red. First run shows full results; subsequent runs show diffs only. Ctrl+C to stop.

### Compare mode (`--compare A B`)
Queries two domains and prints a side-by-side table for all record types. Identical values are dimmed; values unique to one side are highlighted in red. A summary line lists which types differ. The table auto-sizes to the terminal width.
...
python3 dns_lookup.py example.com -r A MX TXT NS
...
# Analyse raw email headers (paste when prompted, Ctrl+D to finish)
...
# Watch for DNS changes every 30 seconds
...
# Save output to file without color
python3 dns_lookup.py -f domains.txt --no-color --output results.txt
...
- `--cert-chain` uses the `openssl` CLI if installed, otherwise falls back to the Python `ssl` module.
- `--compare` and `--mail-headers` do not require a domain argument.
