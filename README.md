...
| `--dkim-selector SEL` | Extra DKIM selector to check |
...
**SPF** — detects `+all` / `~all` / `-all`, lists includes, warns on lookup count >10.  
**DMARC** — reports policy (`none`/`quarantine`/`reject`), `pct=` coverage, missing `rua=`.  
**DKIM** — probes 15 common selectors automatically. Add `--dkim-selector` for a known one.  
...
### Blacklist / RBL (`--rbl`)
Checks domain's A record IPs against 12 blacklists in parallel including Spamhaus ZEN, SpamCop, Barracuda, SORBS, and others.

### CDN / hosting detection (`--cdn`)
Identifies provider from NS, A, and CNAME patterns. Covers Cloudflare, AWS CloudFront, Azure, Fastly, Akamai, Google Cloud, Vercel, Netlify, GitHub Pages, Bunny CDN, Sucuri, and more.
...
Measures A record query response time against 6 public resolvers plus all authoritative nameservers. Displays an ASCII bar chart — green <100ms, yellow <250ms, red >250ms.
...
### File output (`--output`)
Saves full output to a file with ANSI colour codes stripped, while still printing to the terminal.
...
- Propagation, RBL, subdomain, and timing checks run parallel queries for speed.
- `--output` strips ANSI codes so the saved file is clean plain text.
- DKIM probes 15 common selectors; use `--dkim-selector` to add a known one.
# dns_lookup.py
